...
class PlotMealGraph:
          ...
          def getMealsRatioFactor(self):
                    ...
                    try:
                              ...
                              res=session.query(distinct(FoodAndBeverage.beneficiary_id).label("bid")).all()   
                              for x in res:
                                  beneficiary_ids.append(x.bid) 
                              for bid in beneficiary_ids:
                                  res = session.query(func.count(FoodAndBeverage).label("counter")).filter(FoodAndBeverage.beneficiary_id==bid).first()
                                  #print bid,int(res.counter)
                                  meals_recorded.append(int(res.counter))

                                  #print res
                              bubblesort(meals_recorded)
                              last_element=meals_recorded[len(meals_recorded)-1] 
                              

                              #now count how many meal have been recorded by this individual
                              res = session.query(func.count(FoodAndBeverage).label("counter")).filter(FoodAndBeverage.beneficiary_id==self.b_id).first()
                              if res.counter is None:
                                   meals=0
                              else:
                                   meals=int(res.counter)
                              #print meals

                              # the ratio is computed based on the number of meals this user recorded vs number of means recorded with a user with highest number of meals. Then it added to the ratio of Fruit and Veges. then the total is divided by 2

                              if last_element==0:
                                   ratio=0.0
                              else:
                                   ratio=float(float(meals)/float(last_element))


                              specie_size["D1"]=ratio #this is for plants' size
                             


                              #now get data points so that we can be 
                              result=self.getDataPoints()
                              result=json.loads(result)

                              total_fruits_veg=float(result["P00"]["D1"])
                              if total_fruits_veg<0.0:
                                   total_fruits_veg=0.0
                              fruits_veg_ratio=float(total_fruits_veg/3)#it is divided by 3 considering that veges play a greater ro
                              
                              ratio=(ratio+fruits_veg_ratio)/2

                              if ratio >1.0:
                                   ratio=1.0
                              ratio=float("{0:.4f}".format(ratio))
                              specie_size["D0"]=ratio # this is for aquatic animals' size
                    ...
                    return(json.JSONEncoder().encode(specie_size))     
          ...
          def getDataPoints(self):
                    ...
                    try:
                              ...
                              if day=="All":
                                   ...
                                   res = session.query(FoodAndBeverage).filter(FoodAndBeverage.beneficiary_id==beneficiary_id).filter(FoodAndBeverage.beneficiary_id==self.b_id).order_by(FoodAndBeverage.date_consumed).all()
                              
                              else:

                                   res = session.query(FoodAndBeverage).filter(FoodAndBeverage.beneficiary_id==beneficiary_id).filter(FoodAndBeverage.beneficiary_id==self.b_id).filter(FoodAndBeverage.date_consumed>=startdate).filter(FoodAndBeverage.date_consumed<=enddate).order_by(FoodAndBeverage.date_consumed).all()
