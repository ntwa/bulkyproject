...
class RetrievePoints:
    ...
    def getSteps(self,beneficiary_id):
        ...
        try:
            ...
            res=session.query(func.sum(PhysicalActivity.stepscounter).label("sum_steps")).filter(PhysicalActivity.beneficiary_id==beneficiary_id).filter(PhysicalActivity.datecaptured>=last_two_days).filter(PhysicalActivity.datecaptured<=currentdate).first()
            ...
            res=session.query(func.sum(PhysicalActivity.stepscounter).label("sum_steps")).filter(PhysicalActivity.beneficiary_id==beneficiary_id).filter(PhysicalActivity.datecaptured>=datestarted).filter(PhysicalActivity.datecaptured<=currentdate).first()
            ...
            result["dates_counter"]=no_of_days
    ...
    def checkPastThreeDaysSteps(self):
        result={}
        try:
        ...
        for record in intermediaries_tuple.items():
             ...
             if(user["D2"]=="None"):
                 continue
                  ...
                  #averagestepspoints=int(ressteps["steps"]/(100*ressteps["dates_counter"]))
                  stepspoints_last_three_days=int(ressteps["steps1"])
                  stepspoints_average=int(ressteps["steps2"]/ressteps["dates_counter"])
                  ...
                  bonus_points_end.append(stepspoints_average)
...
for rec,tup in decoded_result.items():
  ...
  if (stepspoints[0]==0):
      ...
      recommendations="Hello %s, your %s is currently walking an average of %s steps per day . Please ensure that she has  uploaded all steps. Go to your %s's phone and open an app called Pedometer. Then click upload steps."%(tup["D0"],tup["D4"],stepspoints[1],tup["D4"])
      msg=msg+recommendations
