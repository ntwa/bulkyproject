...
var downloadReminderTemplate=function(opid){
     ...
     //choose whether a template should contain existing data or not.
      ...
      if(opid==1);
      {
        ...
        //now get the groups that are in the existing remidenders
               ...
               {//start for loop
                  ...
                  if(hidden_campaign_id==retrieved_campaign_id){


                    var targeted_audience=resjson[""+x+""]["TargetedAudience"]; //get all groups that were targeted
                    
                    break;//break if we have found the campaign we are looking for
          ...
          if(targeted_audience==null){

            alert("Error: The ID for this campaign doesn't exist.")
            return;
          }


           extra_json={Existing:1,Groups:targeted_audience}; //if Existing is 1; the it means this campaign may have existing indidualize

      }
      else
      {
        extra_json={Existing:0,Groups:{}};
                  ...
                  if(campaign_id==retrieved_campaign_id){
                        ...
                        for(var i=0;i<categories.length;i++){
                            ...
                            //downloader_content=downloader_content+"<button style=\"font-size:24px;cursor:pointer" onclick="downloadReminderTemplate(0);"> <i class="fa fa-download" style="font-size:48px;color:GREEN"></i></button><br><br></td>
                             ...
                             //The show the option for downloading file with reminders.
                             $(".remindertemplates").show();
